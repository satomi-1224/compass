...
- 着手前 `- [ ]` / 完了 `- [x]`。完了にするのは**実機で動作確認できたとき**のみ
  - 実装しただけでは閉じない。各フェーズの末尾に**「実機で確認する」を独立した
    項目**として置き、そこが閉じるまでフェーズ全体は完了にしない
...
- [x] グローバルホットキー登録の方式を決める → **Carbon `RegisterEventHotKey`**
  - CGEvent の event tap と違って**アクセシビリティ権限が要らず**、他アプリの
    キー入力も覗かない。登録した組み合わせが押されたときだけ通知が来る
- [x] `trigger` のパース（`"cmd+alt+shift"` → 修飾キーのビットマスク）
  - Carbon のマスクは `NSEvent.ModifierFlags` と別の値。取り違えると別の組み合わせになる
- [x] 単キーのパース（`t` / `space` / `return` / `delete` → キーコード）
  - 値は Carbon の `kVK_*` から取る。数字をハードコードすると取り違えても気づけない
  - 不明なキー名は Phase 1 のバリデーションでエラーにする
  - トリガーを無視した個別指定は**受け付けない**（例外なしの方針）
- [x] トリガー + 単キーを組み合わせて登録する
  - **差分を取らず毎回すべて登録し直す。** トリガーが変わったときに古い登録が残らない
- [x] `[commands]` の外部コマンド実行（`/bin/sh -c` 経由。`~` や `&` が使える）
- [x] `[actions]` の組み込みアクションのディスパッチ（`search` / `clipboard` / `snippets` の 3 つのみ）
- [x] ホットキー登録の衝突・失敗を検出して通知する
- [x] **自分が登録したキーだけを扱う**
  - ハンドラは `GetApplicationEventTarget()` に付くため、プロセス内の別の
    フレームワークが登録したホットキーのイベントも届く。4 文字コードで突き合わせる
  - 未知の id では `eventNotHandledErr` を返し、イベントを飲み込まない
- [x] 登録が成功することを実機で確認（Hammerspoon が動いた状態でも 4/4 件）
- [ ] **実機で発火を確認する**（キーを押して実際にコマンドが走るか）
  - 現行の全ホットキーの移植は Phase 6 の突き合わせ表で管理する

> `⌘⌥⇧+Space` は現行では Claude.app の起動に使われている**が、そのパスに
> Claude.app が無く既に動いていない**（Phase 6 で判明）。**移す対象が無いので
> 検索窓へ明け渡すだけで済む。**
...
- [x] ファイル検索（`NSMetadataQuery`、探索範囲は config の `scopes`）
  ...
  - **`*` と `?` は探す前に落とす。** 長さの判定・Spotlight のパターン・fuzzy の
    並べ替えのすべてに同じ文字列を使う（片方だけだと `f a*b` が必ず 0 件になる）
  - 当たりが多いと 2000 件で切る。**最近更新した順**に取るので切り取りが決定的
  - ASCII 1 文字では探さないが、**漢字やかなの 1 文字は許す**（`f 本` が引ける）
...
- [x] `--print-candidates <クエリ>` — 検索結果を出して終了する
  - **窓もホットキーも権限も要らずに検索を検証できる。** キーワード切替も含めて、
    実際に窓へ出るのと同じ候補が出る
- [x] `--print-keys` / `--print-placeholders` — 設定に書ける語彙を出す
...
- [ ] スニペットのペースト（同上）
- [ ] `body_command` の出力が貼られること（`git branch --show-current` など）

確認用の設定を `~/.config/compass/` に置いてある。**Hammerspoon が使っていないキー**に
割り当てているので、並行運用のまま試せる（本番の割り当ては [migration.md](./migration.md)）。
...
ホットキーを押さずに窓だけ出す場合は `--show-search` / `--show-clipboard` /
`--show-snippets` を使う。
